...
module.exports = class Generator {
    ...
    transformProfile() {
        ...
        for (let m in this.definition.models) {
            let transformedName = this.transform.modelName ? this.transform.modelName(m, {}) : m;
            if (transformedName === m) {
                continue;
            }
            if (this.definition.models[transformedName]) {
                throw new Error(`Cannot transform model named '${m}'. The transformed name is '${transformedName}', which already exists.`)
            }
            let modelValue = this.definition.models[m];
            delete this.definition.models[m];
            this.definition.models[transformedName] = modelValue;
        }

        // Transform all enum names in the definition
        for (let e in this.definition.enums) {
            let transformedName = this.transform.modelName ? this.transform.modelName(e, {}) : e;
            if (transformedName === e) {
                continue;
            }
            if (this.definition.enums[transformedName]) {
                throw new Error(`Cannot transform enum named '${e}'. The transformed name is '${transformedName}', which already exists.`);
            }
            let enumValue = this.definition.enums[e];
            delete this.definition.enums[e];
            this.definition.enums[transformedName] = enumValue;
        }

        // Update all enum/complex property types in the models
        for (let m in this.definition.models) {
            for (let p in this.definition.models[m]) {
                let property = this.definition.models[m][p];
                if (property.enum) {
                    if (this.transform.modelName) {
                        property.enum = this.transform.modelName(property.enum, {});
                    }
                } else if (property.complex) {
                    if (this.transform.modelName) {
                        property.complex = this.transform.modelName(property.complex, {});
                    }
                }
            }
        }
        ...
        for (let s in this.definition.services) {
            let service = this.definition.services[s];
            for (let o in service) {
                ...
                for (let i = 0; i < operation.parameters.length; i++) {
                    if (operation.parameters[i].dataType.enum) {
                        if (this.transform.modelName) {
                            operation.parameters[i].dataType.enum = this.transform.modelName(operation.parameters[i].dataType.enum, {});
                        }
                    } else if (operation.parameters[i].dataType.complex) {
                        if (this.transform.modelName) {
                            operation.parameters[i].dataType.complex = this.transform.modelName(operation.parameters[i].dataType.complex, {});
                        }
                    }
                }
                for (let r in operation.responses) {
                    ...
                    if (response.dataType) {
                        if (response.dataType.enum) {
                            if (this.transform.modelName) {
                                response.dataType.enum = this.transform.modelName(response.dataType.enum);
                            }
                        } else if (response.dataType.complex) {
                            if (this.transform.modelName) {
                                response.dataType.complex = this.transform.modelName(response.dataType.complex);
                            }
                        }
                    }
                }
