...
module.exports = class Generator {
    ...
    generateImplementations() {
        ...
        for (let i = 0; i < sortedKeys.length; i++) {
            ...
            this.code.push(
                ...
                `        public static $inject: string[] = ['$http', '$q', '${baseUrlProvider}'];`,
                `        constructor (private $http: ng.IHttpService, private $q: ng.IQService, private ${baseUrlProvider}: ${baseUrl.type}) {`,
                `        }`
            );
            ...
            for (let operationName in service) {
                ...
                if (queryParams.length > 0) {
                    this.code.push(`            let queryParams: {[key: string]: any} = {`);
                    for (let i = 0; i < queryParams.length; i++) {
                        let isLastParam = i === queryParams.length - 1;
                        this.code.push(`                 ${queryParams[i].name}: ${queryParams[i].name}${isLastParam ? '' : ','}`);
                    }
                    this.code.push(`            };`)
                }
                ...
                if (queryParams.length > 0) {
                    this.code.push(
                        `                        url: buildServiceUrl(this.${baseUrlPath}, resourceUrl, queryParams)`
                    );
                } else {
                    this.code.push(
                        `                        url: buildServiceUrl(this.${baseUrlPath}, resourceUrl)`
                    );
                }
                this.code.push(
                    `                    }).then((response: ng.IHttpPromiseCallbackArg<${returnType}>) => {`,
    ...
    generateBuildUrlFunction() {
        this.code.push(
            `    function buildServiceUrl(baseUrl: string, resourceUrl: string, queryParams?: {[key: string]: any}): string {`,
            ...
            `                    url += \`\${separator}\${p}=\${encodeURIComponent(queryParams[p])}\`;`,
            `                    isFirst = false;`,
            `                }`,
            `            }`,
            `        }`,
    ...
    generateModels() {
        ...
        let compatibility = this.profile.options.compatibility || {};
        this.code.push(
